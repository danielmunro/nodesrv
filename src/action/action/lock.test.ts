...
describe("lock action", () => {
  ...
  beforeEach(async () => {
    ...
    definition = actionCollection.getMatchingHandlerDefinitionForRequestType(RequestType.Lock)
  })
  ...
  it("should require arguments", async () => {
    ...
    // then
    expect(response.message.getMessageToRequestCreator()).toBe(Messages.All.Arguments.Lock)
  })
  ...
  it("should require a key to be able to lock a locked door", async () => {
    ...
    expect(response.message.getMessageToRequestCreator()).toBe(Messages.Lock.Fail.NoKey)
  ...
  it("should not be able to lock an already locked door", async () => {
    ...
    // then
    expect(response.status).toBe(ResponseStatus.ActionFailed)
    expect(response.message.getMessageToRequestCreator()).toBe(Messages.Lock.Fail.AlreadyLocked)
  })
})
